/**
 * Minimal JSON-schema checker (test oracle helper, T031) — adapted from the
 * claude-code emitter's mini-schema for the vendored opencode config schema
 * (draft 2020-12, root `$ref: "#/$defs/Config"`).
 *
 * Deliberately tiny and dependency-free: just enough to verify that emitted
 * config keys exist in the vendored schema and that basic types match.
 * External `$ref`s (the schema decorates model strings with
 * `https://models.dev/model-schema.json#/...`) are ignored — the node's own
 * inline keywords (`type: "string"`) still apply. NOT a general validator.
 */
...
function resolve(node: SchemaNode, root: RootSchema): SchemaNode {
  ...
  // External $ref (e.g. models.dev) — validate against the inline keywords.
  if (!node.$ref.startsWith("#")) return { ...node, $ref: undefined };
  throw new Error(`mini-schema: unresolvable $ref "${node.$ref}"`);
}
...
/** Build a plain config object from merge-tier KeyPathPatch entries (for oracle validation). */
