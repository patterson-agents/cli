...
> Checkboxes reconciled against git history on 2026-08-12: T001–T028 (incl. T025b) were
> left unticked although the work landed in commits 7bba8eb…38b8ab3, with the phase demo
> transcripts in `artifacts/p2a-demo.txt` … `p8-demo.txt` as evidence.
...
- [X] T001 Snapshot socket scores for planned deps (zod, citty, @clack/prompts,
...
- [X] T002 Remove OpenTUI app: delete src/index.ts, drop @opentui/* + solid-js from
      package.json, re-lock with bun install, prune stale node_modules dirs
- [X] T003 Convert repo root to Bun workspaces: root package.json (workspaces:
      ["packages/*"], private), shared tsconfig.base.json (strict flags preserved),
      per-package tsconfig extends
- [X] T004 [P] Add oxlint config (.oxlintrc.json) and lefthook.yml (pre-commit:
      oxlint + tsc --noEmit; commit-msg: commitlint conventional) at repo root
- [X] T005 [P] Update AGENTS.md (CLAUDE.md symlink intact) with workspace layout,
      spec-tree pointers (specs/001-patterson-cli-v1/), and the constitution's
      never-write list; remove stale OpenTUI sections
- [X] T006 Create empty workspace packages with package.json + src/index.ts + test/
...
- [X] T007 Verify P0 gate: bunx tsc --noEmit, bun test (empty suites pass), oxlint
...
- [X] T008 S1 spike in packages/mcp/spike/: minimal sdk@^1.30 stdio server (2 dummy
...
- [X] T009 [P] Golden tests for IR schema validation in packages/core/test/ir.test.ts
      (valid fixture, unknown-target rejection, SecretRef string-forbidden rule,
      skill name regex, reserved MCP names, argv non-empty)
- [X] T010 IR schemas in packages/core/src/ir/ per data-model.md: PattersonProject,
...
- [X] T011 Config loader in packages/core/src/config.ts: patterson.config.ts (Bun
...
- [X] T012 [P] Registry tests in packages/core/test/registry.test.ts: descriptor
...
- [X] T013 Command registry in packages/core/src/registry/: CommandDescriptor,
...
- [X] T014 [P] Emit engine tests in packages/core/test/emit.test.ts: FileOp apply for
...
- [X] T015 Emit engine in packages/core/src/emit/: FileOp, provenance store
...
- [X] T016 Check registry in packages/core/src/checks/: CheckDef {id, run(ctx) →
      finding[]}, aggregation, --json output shape; used by doctor/check/tutor;
      tests in packages/core/test/checks.test.ts
- [X] T017 definePattersonPlugin INTERFACE (types only: contributes commands/emitters/
      checks/generators/lessons) in packages/core/src/plugin.ts; loading deferred to
      P8 (S4)
- [X] T018 Verify P1 gate: tsc + bun test + oxlint green; commit "P1: core + S1"
...
- [X] T019 [P] S2 spike: fetch json.schemastore.org/claude-code-settings.json; if
...
- [X] T020 [P] S3 spike: empirically test whether Claude Code discovers a skill in
...
- [X] T021 [US1] Fixture suite for claude-code emitter in
...
- [X] T022 [US1] claude-code emitter in packages/emitters/claude-code/src/: settings
...
- [X] T023 [US2] Round-trip drift tests in packages/emitters/claude-code/test/
...
- [X] T024 [US1] `create` skeleton in packages/cli/src/commands/create.ts +
...
- [X] T025 [US2] `sync` + `doctor` + `check` descriptors in
...
- [X] T025b [US2] `init` (adoption) descriptor in packages/cli/src/commands/init.ts:
...
- [X] T026 [US1] Wizard STEP REGISTRY shell in packages/cli/src/wizard/: ordered step
...
- [X] T027 Walking-skeleton E2E test in packages/cli/test/skeleton.e2e.test.ts:
...
- [X] T028 Verify P2a gate: all green + E2E; demo transcript saved to
